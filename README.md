...
| `eval_daemon.batch_size_divisor` | `1` | Divides the daemon's per-bucket eval batch (÷) on the 2nd GPU; grad-accum is ×'d by the same factor, so the logical batch and the metric are unchanged. `1` = full batch (the daemon's loss forward is `no_grad`, with ample 4090 headroom); raise only if VRAM-constrained. |
| `eval_metrics` | `["wer","sim_o"]` | Objective metrics on the fixed refs: `wer` (HuBERT-CTC, no LM) and `sim_o` (WavLM-Large-SV speaker cosine vs. the prompt). Drop either to skip it. |
...
For fast development iteration, `+experiment=overfit_test` (1k clips) caps the **train** split to a small slice via `max_train_clips` (preprocessed before training), so it touches only a tiny fraction of the ~800 GB train data. The default no-arg invocation (the `full_run` composition) uses the full train split.
...
- **Disk** — the binding constraint for the full MLS run; see [§3](#3-dataset-and-custom-mounts). The small-`max_train_clips` diagnostic mode (`+experiment=overfit_test`) needs only a tiny fraction and is the fastest end-to-end check of a fresh setup.
