"""Compile-mode A/B benchmark + per-mode shape-leak (plateau) check.

GPU harness (dummy per-bucket batches, real fwd+bwd+opt step):
...
The shuffled round-robin also exercises the allocator across all bucket sizes back-to-back, so an OOM
is caught per-mode, reported with the memory summary, and recorded as data ("this mode doesn't fit the
current batch budget") rather than killing the sweep. For a dedicated allocator/fragmentation stress
test (independent of compile mode), see stress_test_fragmentation.py.
...
def benchmark_compile_modes(cfg: DictConfig):
    log_file = PROJECT_ROOT / "logs" / "benchmarks" / "benchmark_compile_modes.log"
