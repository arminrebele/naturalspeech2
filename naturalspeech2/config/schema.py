...
class MelConfig:
    ...
    # Per-channel μ/σ (len n_mels) standardizing the dB log-mel that feeds the aligner. None →
    # identity. Generated by scripts/compute_mel_stats.py. Default None = compat shim: ckpts
    # predating the key trained without standardization (matches base.yaml).
...
class EMAConfig:
    enabled: bool = True
    halflife_kimg: float = 50.0   # compat shim: pre-key-era value (base.yaml is authoritative for new runs)
...
class DiffusionModelConfig:
    ...
    time_dim: int = 128
    timestep_embedding_scale: float = 1.0  # compat shim: old ckpts have no key — 1.0 reconstructs them
    beta_min: float = 0.1
    beta_max: float = 20.0
    ...
    t_sampling: str = "uniform"       # "uniform" | "logit_normal"; uniform = compat shim for pre-knob ckpts
...
class AlignerLossWeights:
    ...
    bin_loss: float = 1.0   # compat shim: pre-down-weight-era value (base.yaml: 0.1)
...
@dataclass
class LossWarmups:
    """Values ≠ base.yaml are compat shims (pre-key-era warmups); base.yaml is authoritative for new runs."""
