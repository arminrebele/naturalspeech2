...
@dataclass
class EncodecConfig:
    # Path to per-channel latent normalization stats (mean, std of length 128).
    # Generated by `scripts/compute_encodec_latent_stats.py`. None disables normalization
...
class LossBalanceTargets:
    """Target contribution *shares* per loss leaf — consumed only by the
    loss-analysis run to suggest loss_weights, never read at train time. Flat
    leaves and each group's `group_target` sit on one comparable top-level scale;
    a group's sub-targets are its within-group split. See
    compute_suggested_loss_weights in scripts/train.py.
    """
    duration_predictor_loss: float = 1.0
    pitch_predictor_loss: float = 1.0
    pitch_voicing_loss: float = 1.0
